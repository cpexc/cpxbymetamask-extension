...
const createJsonCpxClient = require('./createJsonCpxClient')
...
const CPX_PROVIDER_TYPES = [CPX_MAINNET, CPX_TESTNET]
...
module.exports = class NetworkController extends EventEmitter {
  ...
  async setProviderType (type, rpcTarget = '', ticker = 'ETH', nickname = '') {
    ...
    assert(CPX_PROVIDER_TYPES.includes(type) || INFURA_PROVIDER_TYPES.includes(type) || type === LOCALHOST, `NetworkController - Unknown rpc type "${type}"`)
    
    // add
    if (type == CPX_MAINNET) 
      ticker = 'CPX',
      rpcTarget = CPX_MAINNET_RPC_URL
    else if(type ==CPX_TESTNET)
      ticker = 'CPX',
      rpcTarget = CPX_TESTNET_RPC_URL
  ...
  _configureProvider (opts) {
    ...
    // add 
    const isCpx = CPX_PROVIDER_TYPES.includes(type)
    
    if (isInfura) {
      this._configureInfuraProvider(opts)
    // add cpx
    } else if (isCpx) {
      this._configureCpxStandardProvider({ type, chainId, ticker, nickname })
  ...
  //add cpx
  _configureCpxStandardProvider ({ type, chainId, ticker, nickname }) {

    var rpcUrl
    if (type == CPX_MAINNET) 
      rpcUrl = CPX_MAINNET_RPC_URL
    else if(type ==CPX_TESTNET)
      rpcUrl = CPX_TESTNET_RPC_URL
